...
class Update_task_status(views.MethodView):
    def post(self):
        ...
        if new_job_status in ['in queue', 'calculating','complete','failed']:
            pass
        else:
            return jsonify({"result": False, "message": "new_job_status has to be 'in queue','calculating','complete' or 'failed'"})
        ...
        try:
            ...
            if new_job_status == 'complete' and old_task_status != 'complete':
